...
public class PetitionController {
    ...
    public ModelAndView toPetitionSubmit(Integer petitionId) {
        ...
        if (redisTemplate.hasKey("departmentList")) {
            // 读取Redis缓存区数据
            departmentList = listOperations.range("departmentList", 0, -1);
            logger.info("读取缓存区数据");
        } else {
            // 获取部门列表
            departmentList = userService.getDepartmentList();
            // 设置至Redis缓存区
            listOperations.leftPushAll("departmentList", departmentList);
            logger.info("缓存区数据同步");
        }
    ...
    public List<User> getTwoPrincipalListByDepartment(String department) {
        ...
        // 判断是否有该部门的缓存
        if (redisTemplate.hasKey(department)) {
            // 读取缓存数据
            principalList = listOperations.range(department, 0,-1);
        } else {
            // 从数据库中查询
            principalList = userService.getTwoPrincipalListByDepartment(department);
            if (principalList != null && principalList.size() > 0) {
                // 同步至缓存区
                listOperations.leftPushAll(department, principalList);
            }
        }
